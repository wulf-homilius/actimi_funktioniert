...
# ---------------------------------------------------------------------------
# Encounter: ein Encounter pro 15-Minuten-Messgruppe
# ---------------------------------------------------------------------------
...
def create_communication(base_url: str, auth: requests.auth.HTTPBasicAuth, *, patient_id: str, observation_ids: List[str], observation_codes: List[str], encounter_ref: Optional[str], settings: Settings, dry_run: bool) -> bool:
    ...
    communication: Dict[str, Any] = {
        ...
        "reasonCode": [{
            "coding": [{"system": "http://www.nursiti.com/notificationReason", "code": "add"}],
            "text": "Ein neues Vitalzeichen wurde hinzugef\u00fcgt",
        }],
...
def _sync_patient(
    ...
    # Gruppierung: alle Messungen im selben 15-Minuten-Slot → ein Encounter + eine Communication
    GROUP_MINUTES = 15

    def group_key_for(dt: datetime) -> str:
        slot = (dt.hour * 60 + dt.minute) // GROUP_MINUTES
        return f"{dt.strftime('%Y-%m-%d')}_{slot:04d}"

    # Schritt 1: valide Items sammeln und in 15-Min-Gruppen einteilen
    ...
    # Schritt 2: pro Gruppe → ein Encounter + alle Observations + eine Communication
    ...
    if gkey not in encounter_cache:
        ...
        # Eine einzige Communication pro Gruppe mit ALLEN Observations
